...
import com.oltpbenchmark.benchmarks.tpccnorpc.procedures.TPCCProcedure;
...
public class TPCCnorpcWorker extends Worker<TPCCnorpcBenchmark> {

    private static final Logger LOG = LoggerFactory.getLogger(TPCCnorpcWorker.class);
    ...
    public TPCCnorpcWorker(TPCCnorpcBenchmark benchmarkModule, int id,
                      int terminalWarehouseID, int terminalDistrictLowerID,
                      int terminalDistrictUpperID, int numWarehouses, int mergeSize) {
    ...
    protected TransactionStatus executeWork(Connection conn, TransactionType nextTransaction) throws UserAbortException, SQLException {
        try {
            ...
            proc.run(conn, gen, terminalWarehouseID, numWarehouses, mergeSize,
