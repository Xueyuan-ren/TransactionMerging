...
package com.oltpbenchmark.benchmarks.tpccnorpc;

import com.oltpbenchmark.WorkloadConfiguration;
import com.oltpbenchmark.api.BenchmarkModule;
import com.oltpbenchmark.api.Loader;
import com.oltpbenchmark.api.Worker;
import com.oltpbenchmark.benchmarks.tpccnorpc.procedures.NewOrder;
...
public class TPCCnorpcBenchmark extends BenchmarkModule {
    private static final Logger LOG = LoggerFactory.getLogger(TPCCnorpcBenchmark.class);

    public TPCCnorpcBenchmark(WorkloadConfiguration workConf) {
        super(workConf);
    }
    ...
    protected List<Worker<? extends BenchmarkModule>> makeWorkersImpl() {
        ...
        try {
            List<TPCCnorpcWorker> terminals = createTerminals();
    ...
    @Override
    protected Loader<TPCCnorpcBenchmark> makeLoaderImpl() {
        return new TPCCLoader(this);
    }

    protected List<TPCCnorpcWorker> createTerminals() throws SQLException {

        TPCCnorpcWorker[] terminals = new TPCCnorpcWorker[workConf.getTerminals()];
        ...
        for (int w = 0; w < numWarehouses; w++) {
            ...
            for (int terminalId = 0; terminalId < numWarehouseTerminals; terminalId++) {
                ...
                // upperDistrictId could be 0, for delivery transaction this can cause error for merging
                if (upperDistrictId <= 0) {
                    upperDistrictId = 1;
                }

                TPCCnorpcWorker terminal = new TPCCnorpcWorker(this, workerId++, w_id, lowerDistrictId, upperDistrictId, numWarehouses, mergeSize);
