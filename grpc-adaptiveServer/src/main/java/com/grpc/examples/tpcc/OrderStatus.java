...
import com.grpc.examples.tpcc.pojo.Oorder;
import com.grpc.examples.OrderStatusReply;
import com.grpc.examples.OrderStatusRequest;
...
public class OrderStatus extends Procedure {
    ...
    private OrderStatusReply.Builder response = OrderStatusReply.newBuilder();
    ...
    public OrderStatusReply orderStatusTransaction(OrderStatusRequest request, Connection conn) throws SQLException {

        int w_id = request.getTerminalWarehouseID();
        int d_id = request.getDistrictID();

        boolean c_by_name = request.getCByName();
        String c_last = request.getCLast();
        int c_id = request.getCId();
    ...
    private Oorder getOrderDetails(Connection conn, int w_id, int d_id, Customer c) throws SQLException {
        try (PreparedStatement ordStatGetNewestOrd = this.getPreparedStatement(conn, ordStatGetNewestOrdSQL)) {
            ...
            try (ResultSet rs = ordStatGetNewestOrd.executeQuery()) {
                ...
                response.setOId(o.o_id)
                        .setOCarrierId(o.o_carrier_id)
                        .setOEntryD((o.o_entry_d).toString());
        ...
        try (PreparedStatement ordStatGetOrderLines = this.getPreparedStatement(conn, ordStatGetOrderLinesSQL)) {
            ...
            try (ResultSet rs = ordStatGetOrderLines.executeQuery()) {
                ...
                while (rs.next()) {
                    ...
                    response.addOrderlines(sb.toString());
                }
            }
