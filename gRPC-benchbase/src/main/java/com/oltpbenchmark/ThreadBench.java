...
public class ThreadBench implements Thread.UncaughtExceptionHandler {
    ...
    private final ArrayList<LatencyRecord.Sample> successSamples = new ArrayList<>();
    ...
    private int[] finalizeWorkers(ArrayList<Thread> workerThreads) throws InterruptedException {
        ...
        for (int i = 0; i < workerThreads.size(); ++i) {
            ...
            successRequests += workers.get(i).getSuccessRequests();
    ...
    private Results runRateLimitedMultiPhase() {
        ...
        try {
            ...
            for (Worker<?> w : workers) {
                ...
                for (LatencyRecord.Sample successSample : w.getSuccessLatencyRecords()) {
                    successSamples.add(successSample);
                }
            }
            Collections.sort(samples);
            Collections.sort(successSamples);

            // Compute stats on all the latencies
            int[] latencies = new int[samples.size()];
            int[] successLatencies = new int[successSamples.size()];
            for (int i = 0; i < samples.size(); ++i) {
                latencies[i] = samples.get(i).getLatencyMicrosecond();
            }
            for (int i = 0; i < successSamples.size(); ++i) {
                successLatencies[i] = successSamples.get(i).getLatencyMicrosecond();
            }

            DistributionStatistics stats = DistributionStatistics.computeStatistics(latencies);

            DistributionStatistics successStats = DistributionStatistics.computeStatistics(successLatencies);

            Results results = new Results(measureEnd - start, requests[0], requests[1], stats, successStats, samples, successSamples);
