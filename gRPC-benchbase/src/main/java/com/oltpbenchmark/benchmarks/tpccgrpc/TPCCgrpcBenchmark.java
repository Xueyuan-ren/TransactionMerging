...
import com.oltpbenchmark.benchmarks.tpccgrpc.procedures.NewOrder;
...
public class TPCCgrpcBenchmark extends BenchmarkModule {
    private static final Logger LOG = LoggerFactory.getLogger(TPCCgrpcBenchmark.class);

    public TPCCgrpcBenchmark(WorkloadConfiguration workConf) {
        super(workConf);
    }
    ...
    protected List<Worker<? extends BenchmarkModule>> makeWorkersImpl() {
        ...
        try {
            List<TPCCgrpcWorker> terminals = createTerminals();
    ...
    @Override
    protected Loader<TPCCgrpcBenchmark> makeLoaderImpl() {
        return new TPCCLoader(this);
    }

    protected List<TPCCgrpcWorker> createTerminals() throws SQLException {

        TPCCgrpcWorker[] terminals = new TPCCgrpcWorker[workConf.getTerminals()];
        ...
        for (int w = 0; w < numWarehouses; w++) {
            ...
            for (int terminalId = 0; terminalId < numWarehouseTerminals; terminalId++) {
                ...
                // For delivery transaction, upperDistrictId could be -1, cause error for grpc merging
                if (upperDistrictId <= 0) {
                    upperDistrictId = 1;
                }

                TPCCgrpcWorker terminal = new TPCCgrpcWorker(this, workerId++, w_id, lowerDistrictId, upperDistrictId, numWarehouses);
