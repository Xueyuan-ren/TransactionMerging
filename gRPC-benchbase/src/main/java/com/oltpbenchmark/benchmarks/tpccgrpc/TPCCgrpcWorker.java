...
public class TPCCgrpcWorker extends Worker<TPCCgrpcBenchmark> {

    private static final Logger LOG = LoggerFactory.getLogger(TPCCgrpcWorker.class);
    ...
    public TPCCgrpcWorker(TPCCgrpcBenchmark benchmarkModule, int id,
     ...
     */
    @Override
    protected TransactionStatus executeWork(Connection conn, TransactionType nextTransaction) throws StatusRuntimeException, UserAbortException, SQLException {
        try {
    ...
    protected void shutDownChannel() {
        if (this.channel != null) {
            try {
                //LOG.info("Shut Down the gRPC Channel for this TPCC worker...");
