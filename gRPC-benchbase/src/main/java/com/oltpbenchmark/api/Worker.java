...
public abstract class Worker<T extends BenchmarkModule> implements Runnable {
    ...
    // rxy: global variable to record transaction success status
    private boolean successStatus = false;
    private LatencyRecord successLatencies;
    ...
    public final int getSuccessRequests() {
        return successLatencies.size();
    }
    ...
    public final Iterable<LatencyRecord.Sample> getSuccessLatencyRecords() {
        return successLatencies;
    }
    ...
    public final void run() {
        ...
        successLatencies = new LatencyRecord(workloadState.getTestStartNs());
                ...
                switch (postState) {
                    case MEASURE:
                        ...
                        if (preState == MEASURE && postPhase.getId() == prePhase.getId()) {
                            ...
                            // rxy: record only success transactions
                            if (this.successStatus) {
                                successLatencies.addLatency(transactionType.getId(), start, end, this.id, prePhase.getId());
                            }
        ...
        try {
            ...
            while (retryCount < maxRetryCount && this.workloadState.getGlobalState() != State.DONE) {
                ...
                try {
                    ...
                    if (err_des.contains("UserAbortException")) {
                        // grpc statusRuntimeException wraps UserAbortException
                        status = TransactionStatus.USER_ABORTED;
                        
                        break;
                    } else if (err_des.contains("Retryable SQLException")) {
                        // grpc statusRuntimeException wraps Retryable SQLException
                        LOG.debug(String.format("Retryable SQLException occurred during [%s]... current retry attempt [%d], max retry attempts [%d], .", transactionType, retryCount, maxRetryCount), ex);

                        status = TransactionStatus.RETRY;

                        retryCount++;
                    } else if (err_des.contains("Unretryable SQLException")) {
                        // grpc statusRuntimeException wraps Unretryable SQLException
                        LOG.warn(String.format("SQLException occurred during [%s] and will not be retried....", transactionType), ex);

                        status = TransactionStatus.ERROR;

                        break;
                    } else if (err_des.contains("Transaction RuntimeException")) {
                        // grpc statusRuntimeException wraps Transaction RuntimeException
                        LOG.warn(String.format("Transaction RuntimeException occurred during [%s].", transactionType), ex);

                        status = TransactionStatus.ERROR;

                        break;
                    } else if (err_des.contains("Unexpected SQLException")) {
                        // grpc statusRuntimeException wraps Unexpected SQLException
                        LOG.warn(String.format("Unexpected SQLException occurred during [%s].", transactionType), ex);

                        status = TransactionStatus.ERROR;

                        break;
                    } else {
                        String msg = String.format("Unexpected Grpc StatusRuntimeException in '%s' when executing '%s' on [%s]", this, transactionType, databaseType.name());
     ...
     */
    protected abstract TransactionStatus executeWork(Connection conn, TransactionType txnType) throws StatusRuntimeException, UserAbortException, SQLException;
